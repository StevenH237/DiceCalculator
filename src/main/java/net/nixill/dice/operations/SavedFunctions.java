...
public class SavedFunctions {
    ...
    try {
      ...
      if (pars.size() > i) {
        ...
        throw new DiceCalcException(
            new IndexOutOfBoundsException("The current function doesn't have " + (i + 1) + " parameters."));
      }
  ...
  public static void save(String name, DCEntity ent) {
    try {
      throw new NoSuchFunctionException("You can't overwrite parameters in a function.");
    } catch (NumberFormatException ex) {
