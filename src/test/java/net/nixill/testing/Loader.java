...
public class Loader extends FunctionLoader {
  private static FunctionState LEVEL = new FunctionState(
      ExpressionSplitter.parse("({1,5}^2+{1,5})/2"));
  ...
  private static FunctionState _P    = null;
  ...
  private FunctionState loadState(String name) {
    if (name.equals("level")) {
      ...
      return _U;
    } else if (name.equals("_P")) {
      return _P;
    }
    
    throw new NoSuchFunctionException(
        "There's no function named " + name + ".");
  }
  ...
  public void save(String name, DCEntity ent) {
    if (name.equals("level")) {
      ...
      _P = new FunctionState(ent);
      return;
    }
    
    throw new NoSuchFunctionException(
        "This loader can't save functions of your own choosing.");
  }
